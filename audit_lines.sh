...
# This file is used to transfer the aureport commands into
# a grep command that runs without root privileges.
...
OPEN_COUNT=`grep -c "session opened" /var/log/secure`
CLOSED_COUNT=`grep -c "session closed" /var/log/secure`
SESSIONS=`grep -e "session opened" -e "session closed" -i /var/log/secure`
LAST=`last -10`
